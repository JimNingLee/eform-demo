...
// reorder for little endian	  
    ...
    private static void SetKey(String key) throws Exception {
        md5 Md5 = new md5();
        byte pbUserKey[] = Md5.encrypt(key);
    ...
    public static String encryptToBase64(String plainText) throws Exception {
        md5 md = new md5();
        String pwd = md.encrypt(plainText.getBytes());
        String enc = encryptToBase64(plainText, pwd);
        return SecureKey.EmbedKey(enc, pwd);
    }
    
    public static String encryptToHex(String plainText) throws Exception {
        md5 md = new md5();
        String pwd = md.encrypt(plainText.getBytes());
        String enc = encryptToHex(plainText, pwd);
        return SecureKey.EmbedKey(enc, pwd);
    }
     ...
     * @return
     */
    public static String encryptToBase64(String plainText, String pwd) {
        String ret = "";
        try {
            //16 bit Padding
            int max = 0;
            if (plainText.getBytes().length % pad != 0) {
                max = pad - (plainText.getBytes().length % pad);
            }
            for (int i = 0; i < max; i++) {
                plainText += " ";
            }

            int size = plainText.getBytes().length;
...
//            System.out.println(e);
    ...
    public static String encryptToHex(String plainText, String pwd) {
        String ret = "";
        try {
            //16 bit Padding
            int max = 0;
...
//            System.out.println(e);
    ...
    public static byte[] encrypt(String plainText, String pwd, String encoding) {
        try {
            //16 bit Padding
            int max = 0;
            if (plainText.getBytes(encoding).length % pad != 0) {
                max = pad - (plainText.getBytes(encoding).length % pad);
            }
            for (int i = 0; i < max; i++) {
                plainText += " ";
            }

            int size = plainText.getBytes(encoding).length;
     ...
     * @return
     */
    public static String decrypt2string(byte[] reCipher, String pwd) {
        String ret = "";
        try {
    ...
    public static byte[] decryptFromHex(String reCipher) throws Exception {
        String[] sdata = SecureKey.ExtractKey(reCipher, 32);
        byte[] b = Hex.parseHexaString(sdata[0]);
        b = decrypt(b, sdata[1]);
        return b;
    }
    
    public static byte[] decryptFromBase64(String reCipher) throws Exception {
        String[] sdata = SecureKey.ExtractKey(reCipher, 32);
        byte[] b = Base64Util.decode(sdata[0].getBytes());
        b = decrypt(b, sdata[1]);
        return b;
    }
    
//    public static byte[] decrypt(String reCipher, String pwd) throws Exception {
//        byte[] b = null;
//        if (EncodingMode.equalsIgnoreCase("Base64")) {
//            b = Base64.decode2byte(reCipher);
//        } else {
//            b = Hex.parseHexaString(reCipher);
//        }
//        return decrypt(b, pwd);
//    }

    public static byte[] decrypt(byte[] reCipher, String pwd) {
        byte rePlain[] = null;
        try {
            int size = reCipher.length;
            byte pbPlain[] = new byte[pad];
...
//        String s = "p";
//        String e = Crypt.encryptToHex(s);
//        String d = new String(Crypt.decryptFromHex(e)).trim();
//        
//        System.out.println(e);
