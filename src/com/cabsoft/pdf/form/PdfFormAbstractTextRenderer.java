...
import com.cabsoft.rx.engine.RXParagraph;
import com.cabsoft.rx.engine.RXPrintText;
import com.cabsoft.rx.engine.RXStyledTextAttributeSelector;
import com.cabsoft.rx.engine.RXReportsContext;
import com.cabsoft.rx.engine.TabStop;
import com.cabsoft.rx.engine.type.HorizontalAlignEnum;
import com.cabsoft.rx.engine.util.RXStringUtil;
import com.cabsoft.rx.engine.util.RXStyledText;
import com.cabsoft.rx.engine.util.RXStyledTextUtil;
import com.cabsoft.rx.engine.util.ParagraphUtil;



/**
 * @author Teodor Danciu (teodord@users.sourceforge.net)
 * @version $Id: AbstractTextRenderer.java 5526 2012-08-01 11:36:41Z teodord $
 */
public abstract class PdfFormAbstractTextRenderer
{
	public static final FontRenderContext LINE_BREAK_FONT_RENDER_CONTEXT = new FontRenderContext(null, true, true);

	protected final RXReportsContext rXReportsContext;
	protected final RXStyledTextAttributeSelector noBackcolorSelector;
	protected RXPrintText text;
	protected RXStyledText styledText;
	 ...
	 * 
	 */
	public PdfFormAbstractTextRenderer(
		RXReportsContext rXReportsContext,
		boolean isMinimizePrinterJobSize,
		boolean ignoreMissingFont
		)
	{
		this.rXReportsContext = rXReportsContext;
		this.noBackcolorSelector = RXStyledTextAttributeSelector.getNoBackcolorSelector(rXReportsContext);
	...
	/**
	 * @deprecated Replaced by {@link #AbstractTextRenderer(RXReportsContext, boolean, boolean)}. 
	 */
	public PdfFormAbstractTextRenderer(
		boolean isMinimizePrinterJobSize,
		boolean ignoreMissingFont
	...
	{
		this(DefaultRXReportsContext.getInstance(), isMinimizePrinterJobSize, ignoreMissingFont);
	}
	 ...
	 *
	 */
	public RXStyledText getStyledText()
	{
		return styledText;
	}
	 ...
	 * 
	 */
	public void initialize(RXPrintText text, int offsetX, int offsetY)
	{
		styledText = RXStyledTextUtil.getInstance(rXReportsContext).getStyledText(text, noBackcolorSelector);
...
//		formatWidth = formatWidth < 0 ? 0 : formatWidth;
		...
		//maxFontSizeFinder = MaxFontSizeFinder.getInstance(!RXCommonText.MARKUP_NONE.equals(text.getMarkup()));
	}
	...
	public void render()
	{
		AttributedCharacterIterator allParagraphs =  
			styledText.getAwtAttributedString(rXReportsContext, ignoreMissingFont).getIterator();
		...
		)
	{
		...
		List<Integer> tabIndexes = RXStringUtil.getTabIndexes(lastParagraphText);
...
//				}
	 ...
	 * 
	 */
	public static float getLineHeight(boolean isFirstLine, RXParagraph paragraph, float maxLeading, float maxAscent)
	{
		float lineHeight = 0;
	 ...
	 * 
	 *
	public static float getLineHeight(RXParagraph paragraph, float lineSpacingFactor, int maxFontSize)
	{
		float lineHeight = 0;
		...
		switch(paragraph.getLineSpacing())
		{
			...
			default :
			{
				throw new RXRuntimeException("Invalid line space type: " + paragraph.getLineSpacing());
			}
		}
