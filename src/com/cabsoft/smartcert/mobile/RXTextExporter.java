...
import com.cabsoft.rx.engine.export.RXTextExporterParameter;
import com.cabsoft.rx.engine.util.RXStyledText;
import com.cabsoft.utils.StringUtils;

/**
 * Exports filled reports in plain text format. The text exporter allows users
 * to define a custom character resolution (the number of columns and rows in
 * text format). Since the character resolution is mapped on the actual pixel
 * resolution, every character corresponds to a rectangle of pixels. If a
 * certain text element has a smaller size in pixels (width, height, or both)
 * than the number of pixels that map to a character, the text element will not
 * be rendered. Because of this, users must take some precautions when creating
 * reports for text export. First, they must make sure the page size in
 * characters is large enough to render the report, because if the report pages
 * contain too much text, some of it may be rendered only partially. On the
 * other hand, if the character resolution is too small compared to the pixel
 * resolution (say, a height of 20 characters for a page 800 pixels tall) all
 * texts with sizes smaller than the one needed to map to a character, will not
 * be displayed (in the previous example, a text element needs to be at least
 * 800/20 = 40 pixels tall in order to be rendered).
 * <p>
 * As a conclusion, the text exporter will yield the better results if the space
 * needed for displaying a text is large. So users have to either design reports
 * with few text or export to big text pages. Another good practice is to
 * arrange text elements at design time as similar as possible to a grid.
 * 
 * @see RXExporterParameter
 * @author Ionut Nedelcu (ionutned@users.sourceforge.net)
 * @version $Id: RXTextExporter.java 5526 2012-08-01 11:36:41Z teodord $
 */
public class RXTextExporter extends RXAbstractExporter {
	private static final String TXT_EXPORTER_PROPERTIES_PREFIX = RXPropertiesUtil.PROPERTY_PREFIX + "export.txt.";
	...
	/**
	 * @see #RXTextExporter(RXReportsContext)
	 */
	public RXTextExporter() {
	 ...
	 *
	 */
	public RXTextExporter(RXReportsContext rXReportsContext) {
	...
	public void exportReport() throws RXException {
		...
		String encoding = getStringParameterOrDefault(RXExporterParameter.CHARACTER_ENCODING, RXExporterParameter.PROPERTY_CHARACTER_ENCODING);

		betweenPagesText = (String) parameters.get(RXTextExporterParameter.BETWEEN_PAGES_TEXT);
		if (betweenPagesText == null) {
			betweenPagesText = systemLineSeparator + systemLineSeparator;
		}

		lineSeparator = (String) parameters.get(RXTextExporterParameter.LINE_SEPARATOR);
	 ...
	 *
	 */
	public void setReportParameters() throws RXException {
		charWidth = getFloatParameter(RXTextExporterParameter.CHARACTER_WIDTH, RXTextExporterParameter.PROPERTY_CHARACTER_WIDTH, 0);
		if (charWidth < 0) {
			throw new RXException("Character width in pixels must be greater than zero.");
		} else if (charWidth == 0) {
			pageWidthInChars = getIntegerParameter(RXTextExporterParameter.PAGE_WIDTH, RXTextExporterParameter.PROPERTY_PAGE_WIDTH, 0);
			if (pageWidthInChars <= 0) {
				throw new RXException("Character width in pixels or page width in characters must be specified and must be greater than zero.");
			}

			charWidth = reportexpressPrint.getPageWidth() / (float) pageWidthInChars;
		} else {
			pageWidthInChars = (int) (reportexpressPrint.getPageWidth() / charWidth);
		}

		charHeight = getFloatParameter(RXTextExporterParameter.CHARACTER_HEIGHT, RXTextExporterParameter.PROPERTY_CHARACTER_HEIGHT, 0);
		if (charHeight < 0) {
			throw new RXException("Character height in pixels must be greater than zero.");
		} else if (charHeight == 0) {
			pageHeightInChars = getIntegerParameter(RXTextExporterParameter.PAGE_HEIGHT, RXTextExporterParameter.PROPERTY_PAGE_HEIGHT, 0);
			if (pageHeightInChars <= 0) {
				throw new RXException("Character height in pixels or page height in characters must be specified and must be greater than zero.");
			}

			charHeight = reportexpressPrint.getPageHeight() / (float) pageHeightInChars;
		} else {
			pageHeightInChars = (int) (reportexpressPrint.getPageHeight() / charHeight);
		}
	}
	...
	protected void exportReportToWriter() throws RXException, IOException {
		for (int reportIndex = 0; reportIndex < rxPrintList.size(); reportIndex++) {
			...
			if (pages != null && pages.size() > 0) {
				...
				/*   */
				setReportParameters();// FIXMENOW check all report level
										// exporter hints and make sure they are
										// read from the current report, not
										// from the first
	...
	/**
	 * Exports a page to the output writer. Only text elements within the page
	 * are considered. For each page, the engine creates a matrix of characters
	 * and each rendered text element is placed at the appropiate position in
	 * the matrix. After all texts are parsed, the character matrix is sent to
	 * the output writer.
	 */
	protected void exportPage(RXPrintPage page) throws IOException {
		List<RXPrintElement> elements = page.getElements();
		...
		for (int i = 0; i < pageHeightInChars; i++) {
			String s = new String(pageData[i]).trim();
	...
	protected void exportElements(List<RXPrintElement> elements) {
	...
	protected void exportText(RXPrintText element) {
		...
		String allText;
		RXStyledText styledText = getStyledText(element);
		if (styledText == null) {
			allText = "";
		} else {
		...
		// uses an array of string buffers, since the maximum number of rows is
		// already calculated
		...
		label: while (lfTokenizer.hasMoreTokens()) {
			...
			if (!line.equals("\n")) {
				...
				while (spaceTokenizer.hasMoreTokens()) {
					...
					// situation: word is larger than remaining space on the
					// current line
					...
					// situation: the word is small enough to fit in the current
					// line
					// in this case just add the word and increment the cursor
					// position
		...
		for (int i = 0; i < rowIndex; i++) {
			...
			// if text is justified, there is no offset, but the line text is
			// modified
	...
	protected int getHeightInChars(int height) {
		// return (int) (((long) pageHeightInChars * height) /
	...
	protected int getWidthInChars(int width) {
		// return pageWidthInChars * width / reportexpressPrint.getPageWidth();
