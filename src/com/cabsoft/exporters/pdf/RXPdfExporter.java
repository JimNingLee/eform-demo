...
/**
 * Exports a ReportExpress document to PDF format. It has binary output type and exports the document to
 * a free-form layout.
 * 
 * @author 김영관 (ykkim@cabsoftware.com)
 * @version $Id: RXPdfExporter.java 5521 2012-07-30 13:25:30Z teodord $
 */
@SuppressWarnings("deprecation")
public class RXPdfExporter extends RXAbstractExporter
{

	private static final Log log = LogFactory.getLog(RXPdfExporter.class);
	...
	public static final String PDF_EXPORTER_PROPERTIES_PREFIX = RXPropertiesUtil.PROPERTY_PREFIX + "export.pdf.";
	 ...
	 */
	public static final String PDF_EXPORTER_KEY = RXPropertiesUtil.PROPERTY_PREFIX + "pdf";
	...
	protected class ExporterContext extends BaseExporterContext implements RXPdfExporterContext
	{
	...
	protected RXPdfExporterTagHelper tagHelper = new RXPdfExporterTagHelper(this);
	 ...
	 *
	 */
	public RXPdfExporter(RXReportsContext rXReportsContext)
	{
	...
	protected Image getPxImage()
	{
		if (pxImage == null)
		{
			try
			{
				pxImage =
					Image.getInstance(
						RXLoader.loadBytesFromResource("com/cabsoft/rx/engine/images/pixel.GIF")
						);
			}
			catch(Exception e)
			{
	...
	public void exportReport() throws RXException
	{
		...
		try
		{
			...
			forceSvgShapes = //FIXME certain properties need to be read from each individual document in batch mode; check all exporters and all props
				getBooleanParameter(
					RXPdfExporterParameter.FORCE_SVG_SHAPES,
					RXPdfExporterParameter.PROPERTY_FORCE_SVG_SHAPES,
					false
					);
			...
			isCompressed = 
				getBooleanParameter(
					...
					RXPdfExporterParameter.PROPERTY_COMPRESSED,
					false
					);
			...
			isEncrypted = 
				getBooleanParameter(
					...
					RXPdfExporterParameter.PROPERTY_ENCRYPTED,
					false
					);
			...
			is128BitKey = 
				getBooleanParameter(
					...
					RXPdfExporterParameter.PROPERTY_128_BIT_KEY,
					false
					);
			...
			pdfformat =
					getStringParameter(
						RXPdfExporterParameter.PDF_FORMAT,
...
//				pdf_format = PdfWriter.PDFX32002;
			...
			Integer permissionsParameter = (Integer)parameters.get(RXPdfExporterParameter.PERMISSIONS);
			...
			tagHelper.setTagged( 
				getBooleanParameter(
					RXPdfExporterParameter.IS_TAGGED,
					RXPdfExporterParameter.PROPERTY_TAGGED,
					false
					)
				);
			
			// no export param for this
			collapseMissingBookmarkLevels = RXPropertiesUtil.getInstance(rxReportsContext).getBooleanProperty(reportexpressPrint, 
			...
			else
			{
				...
				{
					throw new RXException("Error trying to export to file : " + destFile, e);
				}
				finally
				{
					if (os != null)
	...
	{
		pdfFontMap = (Map<FontKey,PdfFont>) parameters.get(RXExporterParameter.FONT_MAP);
	}
	...
	protected void setSplitCharacter()
	{
		...
		Boolean useFillSplitCharacterParam = (Boolean) parameters.get(RXPdfExporterParameter.FORCE_LINEBREAK_POLICY);
		if (useFillSplitCharacterParam == null)
		{
			useFillSplitCharacter = 
				RXPropertiesUtil.getInstance(rxReportsContext).getBooleanProperty(
	...
	protected void exportReportToStream(OutputStream os) throws RXException
	{
		...
		imageTesterDocument =
			new Document(
				new Rectangle(
					10, //reportexpressPrint.getPageWidth(),
		...
		try
		{
			...
			{
				if (RXPdfExporterParameter.PRINT_SCALING_DEFAULT.equals(printScaling))
				{
					pdfWriter.addViewerPreference(PdfName.PRINTSCALING, PdfName.APPDEFAULT);
				}
				else if (RXPdfExporterParameter.PRINT_SCALING_NONE.equals(printScaling))
				{
			...
			String title = (String)parameters.get(RXPdfExporterParameter.METADATA_TITLE);
			if( title != null )
			{
				document.addTitle(title);
			}
			String author = (String)parameters.get(RXPdfExporterParameter.METADATA_AUTHOR);
			if( author != null )
			{
				document.addAuthor(author);
			}
			String subject = (String)parameters.get(RXPdfExporterParameter.METADATA_SUBJECT);
			if( subject != null )
			{
				document.addSubject(subject);
			}
			String keywords = (String)parameters.get(RXPdfExporterParameter.METADATA_KEYWORDS);
			if( keywords != null )
			{
				document.addKeywords(keywords);
			}
			String creator = (String)parameters.get(RXPdfExporterParameter.METADATA_CREATOR);
			if( creator != null )
			{
			...
			{
				document.addCreator("ReportExpress (" + reportexpressPrint.getName() + ")");
			}
			
			// BEGIN: PDF/A support
			String pdfaConformance = getStringParameter( RXPdfExporterParameter.PDFA_CONFORMANCE, RXPdfExporterParameter.PROPERTY_PDFA_CONFORMANCE);
			boolean gotPdfa = false;
			if (pdfaConformance != null && !RXPdfExporterParameter.PDFA_CONFORMANCE_NONE.equalsIgnoreCase(pdfaConformance)) 
			{
				if (RXPdfExporterParameter.PDFA_CONFORMANCE_1A.equalsIgnoreCase(pdfaConformance))
				{
					pdfWriter.setPDFXConformance(PdfWriter.PDFA1A);
					gotPdfa = true;
				}
				else if (RXPdfExporterParameter.PDFA_CONFORMANCE_1B.equalsIgnoreCase(pdfaConformance))
				{
			...
			// BEGIN: PDF/A support
			if (gotPdfa) {
				String iccProfilePath = getStringParameter( RXPdfExporterParameter.PDFA_ICC_PROFILE_PATH, RXPdfExporterParameter.PROPERTY_PDFA_ICC_PROFILE_PATH);
				if (iccProfilePath != null) {
					...
					InputStream iccIs = RepositoryUtil.getInstance(rxReportsContext).getInputStreamFromLocation(iccProfilePath);
			...
			for(reportIndex = 0; reportIndex < rxPrintList.size(); reportIndex++)
			{
				...
				BorderOffset.setLegacy(
					RXPropertiesUtil.getInstance(rxReportsContext).getBooleanProperty(reportexpressPrint, BorderOffset.PROPERTY_LEGACY_BORDER_OFFSET, false)
					);
				
				boolean sizePageToContent = RXPropertiesUtil.getInstance(rxReportsContext).getBooleanProperty(reportexpressPrint, RXPdfExporterParameter.PROPERTY_SIZE_PAGE_TO_CONTENT, false);
		...
		{
			throw new RXException("PDF Document error : " + reportexpressPrint.getName(), e);
		}
		catch(IOException e)
		{
			throw new RXException("Error generating PDF report : " + reportexpressPrint.getName(), e);
		}
		finally
		{
	...
	protected void writePageAnchor(int pageIndex) throws DocumentException 
	{
		...
		FontUtil.getInstance(rxReportsContext).getAttributesWithoutAwtFont(attributes, new RXBasePrintText(reportexpressPrint.getDefaultStyleProvider()));
		Font pdfFont = getFont(attributes, getLocale(), false);
		Chunk chunk = new Chunk(" ", pdfFont);
		
		chunk.setLocalDestination(RX_PAGE_ANCHOR_PREFIX + reportIndex + "_" + (pageIndex + 1));
	 ...
	 *
	 */
	protected void setPageSize(RXPrintPage page) throws RXException, DocumentException, IOException
	{
	 ...
	 *
	 */
	protected void exportPage(RXPrintPage page) throws RXException, DocumentException, IOException
	{
		tagHelper.startPage();
...
//		exportTextBorder(zlist);
		exportElements(zlist);
//		exportTextBorder(els);
		exportElements(els);
	...
	private void exportTextBorder(List<RXPrintElement> list){
		for(Iterator<RXPrintElement> it = list.iterator(); it.hasNext();){
			RXPrintElement element = it.next();
			if (element instanceof RXPrintText){
				RXPrintText text = (RXPrintText)element;
				/* Text의 Border를 그린다.  */
...
//	protected void exportPageOld(RXPrintPage page) throws RXException, DocumentException, IOException
//	{
//		tagHelper.startPage();
//		
//		Collection<RXPrintElement> elements = page.getElements();
...
//	}
	...
	protected void exportElements(Collection<RXPrintElement> elements) throws DocumentException, IOException, RXException
	{
		if (elements != null && elements.size() > 0)
		{
			for(Iterator<RXPrintElement> it = elements.iterator(); it.hasNext();)
				...
				if (line.getHeight() != 1)
				{
					...
					if (line.getLinePen().getLineStyleValue() == LineStyleEnum.DOUBLE)
					{
						pdfContentByte.moveTo(
							...
							reportexpressPrint.getPageHeight() - line.getY() - getOffsetY()
							);
						pdfContentByte.lineTo(
							line.getX() + getOffsetX() + 0.5f - lineWidth / 3,
							reportexpressPrint.getPageHeight() - line.getY() - getOffsetY() - line.getHeight()
							);
				...
				if (line.getHeight() == 1)
				{
					...
					if (line.getLinePen().getLineStyleValue() == LineStyleEnum.DOUBLE)
					{
						pdfContentByte.moveTo(
							...
							reportexpressPrint.getPageHeight() - line.getY() - getOffsetY() - 0.5f + lineWidth / 3
							);
						pdfContentByte.lineTo(
					...
					else
					{
						pdfContentByte.moveTo(
							line.getX() + getOffsetX(),
							reportexpressPrint.getPageHeight() - line.getY() - getOffsetY() - 0.5f
							);
						pdfContentByte.lineTo(
							line.getX() + getOffsetX() + line.getWidth(),
							reportexpressPrint.getPageHeight() - line.getY() - getOffsetY() - 0.5f
					...
					if (line.getDirectionValue() == LineDirectionEnum.TOP_DOWN)
					{
						if (line.getLinePen().getLineStyleValue() == LineStyleEnum.DOUBLE)
						{
							...
							pdfContentByte.moveTo(
								...
								reportexpressPrint.getPageHeight() - line.getY() - getOffsetY() + (float)ytrans
								);
							pdfContentByte.lineTo(
								line.getX() + getOffsetX() + line.getWidth() + (float)xtrans,
								reportexpressPrint.getPageHeight() - line.getY() - getOffsetY() - line.getHeight() + (float)ytrans
								);
	...
	protected void exportRectangle(RXPrintRectangle rectangle)
	{
		...
		if (rectangle.getModeValue() == ModeEnum.OPAQUE)
		{
			pdfContentByte.roundRectangle(
				...
				reportexpressPrint.getPageHeight() - rectangle.getY() - getOffsetY() - rectangle.getHeight(),
				rectangle.getWidth(),
				rectangle.getHeight(),
				rectangle.getRadius()
				);
		...
		if (lineWidth > 0f)
		{
			if (rectangle.getLinePen().getLineStyleValue() == LineStyleEnum.DOUBLE)
			{
				pdfContentByte.roundRectangle(
					...
					reportexpressPrint.getPageHeight() - rectangle.getY() - getOffsetY() - rectangle.getHeight() - lineWidth / 3,
			...
			else
			{
				pdfContentByte.roundRectangle(
					...
					reportexpressPrint.getPageHeight() - rectangle.getY() - getOffsetY() - rectangle.getHeight() + lineOffset,
	...
	protected void exportEllipse(RXPrintEllipse ellipse)
	{
		...
		if (ellipse.getModeValue() == ModeEnum.OPAQUE)
		{
			pdfContentByte.ellipse(
				...
				reportexpressPrint.getPageHeight() - ellipse.getY() - getOffsetY() - ellipse.getHeight(),
				ellipse.getX() + getOffsetX() + ellipse.getWidth(),
				reportexpressPrint.getPageHeight() - ellipse.getY() - getOffsetY()
				);
		...
		if (lineWidth > 0f)
		{
			if (ellipse.getLinePen().getLineStyleValue() == LineStyleEnum.DOUBLE)
			{
				pdfContentByte.ellipse(
					...
					reportexpressPrint.getPageHeight() - ellipse.getY() - getOffsetY() - ellipse.getHeight() - lineWidth / 3,
					ellipse.getX() + getOffsetX() + ellipse.getWidth() + lineWidth / 3,
					reportexpressPrint.getPageHeight() - ellipse.getY() - getOffsetY() + lineWidth / 3
					);
			...
			else
			{
				pdfContentByte.ellipse(
					...
					reportexpressPrint.getPageHeight() - ellipse.getY() - getOffsetY() - ellipse.getHeight() + lineOffset,
					ellipse.getX() + getOffsetX() + ellipse.getWidth() - lineOffset,
					reportexpressPrint.getPageHeight() - ellipse.getY() - getOffsetY() - lineOffset
					);
	 ...
	 */
	@SuppressWarnings("unused")
	public void exportImage(RXPrintImage printImage) throws DocumentException, IOException,  RXException
	{
		...
		if (printImage.getModeValue() == ModeEnum.OPAQUE || !pdfformat.equalsIgnoreCase("PDFXNONE"))
		{
			pdfContentByte.setRGBColorFill(
			...
			pdfContentByte.rectangle(
				...
				reportexpressPrint.getPageHeight() - printImage.getY() - getOffsetY(),
				printImage.getWidth(),
			...
			)
		{
			if (renderer.getTypeValue() == RenderableTypeEnum.IMAGE)
			{
				...
				renderer = RenderableUtil.getInstance(rxReportsContext).getOnErrorRendererForImageData(renderer, printImage.getOnErrorTypeValue());
			}
		}
		else
		{
		...
		if (renderer != null)
		{
			...
			if (renderer.getTypeValue() == RenderableTypeEnum.IMAGE)
			{
				...
				switch(printImage.getScaleImageValue())
				{
					case CLIP :
					{
						...
						// Better to test and replace the renderer now, in case of lazy load error.
						renderer = 
							RenderableUtil.getInstance(rxReportsContext).getOnErrorRendererForDimension(
								renderer, 
						...
						//image = com.cabsoft.text.Image.getInstance(awtImage, printImage.getBackcolor());
					...
					case FILL_FRAME :
					{
						...
						else
						{
							try
							{
								image = Image.getInstance(renderer.getImageData(rxReportsContext));
							...
							catch(Exception e)
							{
								RXImageRenderer tmpRenderer = 
									RXImageRenderer.getOnErrorRendererForImage(
										rxReportsContext,
										RXImageRenderer.getInstance(renderer.getImageData(rxReportsContext)), 
										printImage.getOnErrorTypeValue()
										);
								if (tmpRenderer == null)
								{
								...
								java.awt.Image awtImage = tmpRenderer.getImage(rxReportsContext);
			...
			image.setAbsolutePosition(
				...
				reportexpressPrint.getPageHeight() - printImage.getY() - offsetY - image.scaledHeight() - borderOffset
				);
			...
			*/
			...
			if (chunk != null)
			{
				...
				ColumnText colText = new ColumnText(pdfContentByte);
				int upperY = reportexpressPrint.getPageHeight() - printImage.getY() - topPadding - getOffsetY() - yoffset;
		...
		if (link != null)
		{
			switch(link.getHyperlinkTypeValue())
			{
				...
				case LOCAL_PAGE :
				{
					...
					{
						chunk.setLocalGoto(RX_PAGE_ANCHOR_PREFIX + reportIndex + "_" + link.getHyperlinkPage().toString());
					}
					break;
				}
				case REMOTE_ANCHOR :
	 ...
	 *
	 */
	protected Phrase getPhrase(AttributedString as, String text, RXPrintText textElement)
	{
		...
		while(runLimit < text.length() && (runLimit = iterator.getRunLimit()) <= text.length())
		{
			...
			{
				hyperlink = (RXPrintHyperlink)attributes.get(RXTextAttribute.HYPERLINK);
			}
	...
	protected Font getFont(Map<Attribute,Object> attributes, Locale locale, boolean setFontLines)
	{
		...
		// use the same font scale ratio as in RXStyledText.getAwtAttributedString
		...
		FontKey key = new FontKey(rxFont.getFontName(), rxFont.isBold(), rxFont.isItalic());
		...
		{
			FontInfo fontInfo = FontUtil.getInstance(rxReportsContext).getFontInfo(rxFont.getFontName(), locale);
			if (fontInfo == null)
			{
				//fontName NOT found in font extensions
				pdfFont = new PdfFont(rxFont.getPdfFontName(), rxFont.getPdfEncoding(), rxFont.isPdfEmbedded());
			}
			else
			{
...
//					if (face == null)
//					{
//						throw new RXRuntimeException("Font family '" + family.getName() + "' does not have the normal font face.");
//					}
				}
				else
				{
				...
				if (pdfFontName == null)
				{
					...
					pdfFontName = (face == null || face.getFile() == null ? rxFont.getPdfFontName() : face.getFile());
...
//				if (ttf == null)
//				{
//					throw new RXRuntimeException("The '" + face.getName() + "' font face in family '" + family.getName() + "' returns a null file.");
//				}
				
				pdfFont = 
					new PdfFont(
						pdfFontName, 
						family.getPdfEncoding() == null ? rxFont.getPdfEncoding() : family.getPdfEncoding(),
 						family.isPdfEmbedded() == null ? rxFont.isPdfEmbedded() : family.isPdfEmbedded().booleanValue(), 
						rxFont.isBold() && ((pdfFontStyle & java.awt.Font.BOLD) == 0), 
						rxFont.isItalic() && ((pdfFontStyle & java.awt.Font.ITALIC) == 0)
						);
			}
		}
		...
		try
		{
			if(forecolor!=null){
				font = FontFactory.getFont(
					...
					rxFont.getFontSize() * fontSizeScale,
					pdfFontStyle,
					new BaseColor(forecolor.getRed(), forecolor.getGreen(), forecolor.getBlue(), forecolor.getAlpha())
				);
			}else{
				font = FontFactory.getFont(
						pdfFont.getPdfFontName(),
			...
			if (font.getBaseFont() == null && font.getFamily().equals(Font.UNDEFINED))
			{
				font = null;
			}
		}
		...
		if (font == null)
		{
			...
			try
			{
				bytes = RepositoryUtil.getInstance(rxReportsContext).getBytesFromLocation(pdfFont.getPdfFontName());
			}
			catch(RXException e)
			{
				throw //NOPMD
...
//		System.out.println("styledText = [" + styledText.getText() + "]");
		...
		if (topPen.getLineWidth().floatValue() > 0f)
		{
			...
			if (topPen.getLineStyleValue() == LineStyleEnum.DOUBLE)
			{
				...
				pdfContentByte.moveTo(
					...
					reportexpressPrint.getPageHeight() - element.getY() - getOffsetY() + topOffset / 3
					);
				pdfContentByte.lineTo(
			...
			else
			{
				...
				pdfContentByte.moveTo(
					element.getX() + getOffsetX() - leftOffset,
					reportexpressPrint.getPageHeight() - element.getY() - getOffsetY() - topOffset
					);
				pdfContentByte.lineTo(
					element.getX() + getOffsetX() + element.getWidth() + rightOffset,
					reportexpressPrint.getPageHeight() - element.getY() - getOffsetY() - topOffset
	 ...
	 *
	 */
	protected void exportLeftPen(RXPen topPen, RXPen leftPen, RXPen bottomPen, RXPrintElement element)
	{
		if (leftPen.getLineWidth().floatValue() > 0f)
		{
			...
			if (leftPen.getLineStyleValue() == LineStyleEnum.DOUBLE)
			{
				...
				pdfContentByte.lineTo(
					element.getX() + getOffsetX() - leftOffset / 3,
					reportexpressPrint.getPageHeight() - element.getY() - getOffsetY() - element.getHeight() - bottomOffset
					);
				pdfContentByte.stroke();
				...
				pdfContentByte.lineTo(
					element.getX() + getOffsetX() + leftOffset / 3,
					reportexpressPrint.getPageHeight() - element.getY() - getOffsetY() - element.getHeight() + bottomOffset / 3
					);
				pdfContentByte.stroke();
			}
			else
	 ...
	 *
	 */
	protected void exportBottomPen(RXPen leftPen, RXPen bottomPen, RXPen rightPen, RXPrintElement element)
	{
	 ...
	 *
	 */
	protected void exportRightPen(RXPen topPen, RXPen bottomPen, RXPen rightPen, RXPrintElement element)
	{
	 ...
	 *
	 */
	private static void preparePen(PdfContentByte pdfContentByte, RXPen pen, int lineCap)
	{
	...
	protected static synchronized void registerFonts ()
	{
		if (!fontsRegistered)
		{
			List<PropertySuffix> fontFiles = RXPropertiesUtil.getInstance(DefaultRXReportsContext.getInstance()).getProperties(PDF_FONT_FILES_PREFIX);//FIXMECONTEXT no default here and below
			...
			List<PropertySuffix> fontDirs = RXPropertiesUtil.getInstance(DefaultRXReportsContext.getInstance()).getProperties(PDF_FONT_DIRS_PREFIX);
			if (!fontDirs.isEmpty())
			{
	...
	protected void addBookmark(int level, String title, int x, int y)
	{
		...
		Bookmark bookmark = new Bookmark(parent, x, reportexpressPrint.getPageHeight() - y, title);
		bookmarkStack.push(bookmark);
	}


	protected void setAnchor(Chunk chunk, RXPrintAnchor anchor, RXPrintElement element)
	{
	...
	protected void exportFrame(RXPrintFrame frame) throws DocumentException, IOException, RXException
	{
		if (frame.getModeValue() == ModeEnum.OPAQUE)
		{
			...
			pdfContentByte.rectangle(
				x,
				reportexpressPrint.getPageHeight() - y,
				frame.getWidth(),
				- frame.getHeight()
				);
			pdfContentByte.fill();
	...
	protected void exportGenericElement(RXGenericPrintElement element)
	{
		GenericElementPdfHandler handler = (GenericElementPdfHandler) 
				GenericElementHandlerEnviroment.getInstance(getRXReportsContext()).getElementHandler(
