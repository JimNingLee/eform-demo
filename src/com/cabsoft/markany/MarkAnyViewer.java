...
public class MarkAnyViewer extends RXAbstractExporter {

	private final Log log = LogFactory.getLog(MarkAnyViewer.class);
	...
	public final String PROPERTY_HTML_POSITION = HTML_EXPORTER_PROPERTIES_PREFIX + "position";
	...
	private final int Print_Margin = 40;
	...
	protected class ExporterContext extends BaseExporterContext implements RXHtmlExporterContext {
		...
		public String getHyperlinkURL(RXPrintHyperlink link) {
			return MarkAnyViewer.this.getHyperlinkURL(link);
		}
	}
    ...
    /**
	 * @param useAbsolutePosition the useAbsolutePosition to set
	 */
	public void setUseAbsolutePosition(boolean useAbsolutePosition) {
		this.useAbsolutePosition = useAbsolutePosition;
	}
	
	public void exportReport(String pwd) throws RXException {
		this.pwd = pwd;
	...
	public void exportReport(String webUrl, String pwd) throws RXException {
	...
	public RXPrintImage getImage(List reportexpressPrintList, String imageName) {
		return getImage(reportexpressPrintList, getPrintElementIndex(imageName));
	}

	public RXPrintImage getImage(List reportexpressPrintList, RXPrintElementIndex imageIndex) {
	...
	protected void exportReportToWriter() throws RXException, IOException {
		...
		if (htmlHeader == null) {
			...
			writer.write(MarkAnyViewerHelper.setMaxPages(
					max_pages, 
					toScaledSize(reportexpressPrint.getPageWidth()), 
					toScaledSize(reportexpressPrint.getPageHeight() - Print_Margin)));

			writer.write(MarkAnyViewerHelper.getDocStart(
					(String) parameters.get(RXExporterParameter.USER_AGENT), 
					toScaledSize(reportexpressPrint.getPageWidth()),
					toScaledSize(reportexpressPrint.getPageHeight() - Print_Margin), 
		...
		for (reportIndex = 0; reportIndex < rxPrintList.size(); reportIndex++) {
			...
			if (pages != null && pages.size() > 0) {
				...
				for (pageIndex = startPageIndex; pageIndex <= endPageIndex; pageIndex++) {
					...
					if (htmlHeader != null) {
						writer.write(MarkAnyViewerHelper.getPageStart(
								toScaledSize(reportexpressPrint.getPageWidth()),
								toScaledSize(reportexpressPrint.getPageHeight() - Print_Margin),
								useAbsolutePosition
						));
					...
					if (htmlHeader != null) {
						writer.write(MarkAnyViewerHelper.getPageEnd());
					}
		...
		if (htmlHeader == null) {
			...
			writer.write(MarkAnyViewerHelper.getDocEnd(htmlFooter));
		}
	...
	protected void exportPage(RXPrintPage page) throws RXException, IOException {
		...
		if(useAbsolutePosition==true){
			for(int i=0; i<elements.size(); i++){
	            ...
	            if (element instanceof RXPrintImage) {
	                ...
	                String position = RXProperties.getProperty(image, PROPERTY_HTML_POSITION);
		...
		exportGrid(layout, isWhitePageBackground, getTableSummary());
	 ...
	 *
	 */
	protected void exportGrid(RXGridLayout gridLayout, boolean whitePageBackground, String summary) throws IOException, RXException {
		...
		int pgH = reportexpressPrint.getPageHeight() - Print_Margin;
		int totalH = 0;
		thDepth = 0;
	 ...
	 *
	 */
	protected void exportStyledText(RXStyledText styledText, Locale locale, String id, String key) throws IOException {
		exportStyledText(styledText, null, null, locale, id, key);
	}

	/**
	 *
	 */
	protected void exportStyledText(RXStyledText styledText, String alt, String title, Locale locale, String id, String key) throws IOException {
		...
		while (runLimit < styledText.length() && (runLimit = iterator.getRunLimit()) <= styledText.length()) {
			...
			exportStyledTextRun(iterator.getAttributes(), text.substring(iterator.getIndex(), runLimit), alt, title, locale, id, key);
	 ...
	 *
	 */
	protected void exportStyledTextRun(Map attributes, String text, Locale locale, String id, String key) throws IOException {
		exportStyledTextRun(attributes, text, null, null, locale, id, key);
	}

	/**
	 *
	 */
	protected void exportStyledTextRun(Map attributes, String text, String alt, String title, Locale locale, String id, String key) throws IOException {
		...
		if (id == null || id.equalsIgnoreCase("")) {
			writer.write("<span style=\"" + style.trim() + "\">");
			
			if (text == null || text.equalsIgnoreCase("")) {
				writer.write(RXStringUtil.htmlEncode(" "));
			} else {
				if(key.equalsIgnoreCase("MarkAnyCopyDetector") || key.equalsIgnoreCase("MarkAnyBarcode")){
					writer.write(text);
				}else{
			...
			writer.write("<span id=\"" + getHtmlID(id) + "\" " + desc + " style=\"" + style.trim() + "\">");
			if (text == null || text.equalsIgnoreCase("")) {
				writer.write(RXStringUtil.htmlEncode(" "));
			} else {
				if(key.equalsIgnoreCase("MarkAnyCopyDetector") || key.equalsIgnoreCase("MarkAnyBarcode")){
	...
	protected void exportText(RXPrintText text, RXExporterGridCell gridCell) throws IOException {
		...
		if (textLength > 0) {
			...
			exportStyledText(styledText, alt, title, getTextLocale(text), id, key);
		} else {
			writer.write(emptyCellStringProvider.getStringForEmptyTD(webUrl.equalsIgnoreCase("") ? imagesURI : webUrl));
		}
	...
	protected void exportImage(RXPrintImage image, RXExporterGridCell gridCell) throws RXException, IOException {
		...
		if (renderer != null || isUsingImagesToAlign) {
			...
			if(gridCell==null && useAbsolutePosition){
			...
			if(gridCell==null && useAbsolutePosition) writer.write("</div>");
	...
	protected void exportFrame(RXPrintFrame frame, RXExporterGridCell gridCell) throws IOException, RXException {
		...
		try {
			exportGrid(gridCell.getLayout(), false, "title='" + title + "' summary='" + title + "'");
		} finally {
			if (frameBackcolor != null) {
				restoreBackcolor();
			}
